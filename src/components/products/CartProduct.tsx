...
export default function CartProduct({item, handleRemoveItem,
    handleUpdateProductCartCount} : CartProductProps) {
    ...
    <div key={item._id} className="flex gap-4 p-4 border rounded-lg">
                ...
                <div className="flex flex-col items-end gap-2">
                    <Button onClick={()=> handleRemoveItem(item.product._id , setIsRemovingProduct)} variant="ghost" size="sm">
                    ...
                    <Button disabled={item.count==item.product.quantity}
                    onClick={()=>handleUpdateCount(productCount +1)}                    
                    variant="outline" size="sm">
